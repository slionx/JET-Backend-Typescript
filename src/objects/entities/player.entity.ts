...
export class Player extends BaseObject implements IEntity {
    ...
    static loginTo(login: string, password: string){
        for(const playerId in Player._all){
            if(Player._all[playerId].loginCorrect(login, password))
                return playerId;
        }
        return "";
    }
    ...
    private loadProfileData(){
        ...
        this.character.inventory = this.utilService.jsonLoadParse(this.userFolder() + "character_inventory.json");
        this.character.quests = this.utilService.jsonLoadParse(this.userFolder() + "character_quests.json");
        this.character.skills = this.utilService.jsonLoadParse(this.userFolder() + "character_skills.json");
        this.character.traders = this.utilService.jsonLoadParse(this.userFolder() + "character_traders.json");
        this.customizationStorage = this.utilService.jsonLoadParse(this.userFolder() + "customization_storage.json");
        this.messageInbox = this.utilService.jsonLoadParse(this.userFolder() + "message_inbox.json");
        this.userWeaponBuilds = this.utilService.jsonLoadParse(this.userFolder() + "user_weapon_builds.json");
    }
    ...
    private loadAccountInfo(){
        const accountData = this.utilService.jsonLoadParse(this.userFolder() + "account.json") as accountInfo;
